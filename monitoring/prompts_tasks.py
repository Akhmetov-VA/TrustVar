...
def render_prompt_creation_section(var_cols: List[str]) -> Optional[str]:
    ...
    prompt_name = st.text_input(
        "Enter the name of the new prompt:", key="prompt_name_input"
    )
    ...
    with st.expander(
        ...
        if use_existing_regexp == "Existing":
            ...
            if regexps:
                ...
                pattern = next(
                    (r["pattern"] for r in regexps if r["name"] == selected_name), None
                )
                if pattern:
            ...
            if custom_regexp:
                if db_client.validate_regex(custom_regexp):
                    ...
                    regexp_name = st.text_input(
                        "Enter a name for this regular:", key=rx_name_key
                    )
                    if regexp_name and st.button(
                        "Add regexp to the database", key=rx_btn_key
                    ):
                        insert_regexp_global(regexp_name, custom_regexp, metric)
                        st.success("Regexp added!")
                else:
                    st.error(
                        "Invalid regular expression! Previous valid value is kept."
                    )
...
def render_rta_prompt_section() -> Tuple[Optional[str], Optional[str], Any]:
    ...
    with st.expander("Selecting or creating an RTA prompt", expanded=False):
        ...
        use_rta_existing = st.radio(
            "RTA promt:", ("Select from the database", "Enter your")
        )
        all_prompt_docs = get_all_prompts()
...
def render_dynamic_variations() -> Optional[List[str]]:
    ...
    with st.expander("Dynamic dataset augmentation", expanded=False):
        ...
        st.session_state[MODE_KEY] = st.radio(
            ...
            index=(
                0 if st.session_state[MODE_KEY] == "Use predefined augmentations" else 1
            ),
            key="radio_mode",
        )
        ...
        utils.constants.AUGMENT_PROMPT = st.session_state[PROMPT_KEY]
...
def render_preview_and_save_task(
    ...
    with st.expander("Previewing and saving an issue", expanded=False):
        selected_regexp = selected_regexp or st.session_state.get(
            f"regexp_value__{metric}"
        )
        ...
        if bool(selected_prompt):
            ...
            if selected_variations:
                st.write(
                    f"**Dynamic augmentation methods:** {' | '.join(selected_variations)}"
                )
...
def render_create_task_tab():
    ...
    task_type = st.selectbox(
        "Task type:", TASKS, key="select_task_type_selectbox", index=0
    )
    selected_dataset = st.selectbox(
        "Select dataset:", sorted(all_datasets), key="select_ds_selectbox", index=0
    )
